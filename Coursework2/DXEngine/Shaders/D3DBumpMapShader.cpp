...
D3DBumpMapShader::D3DBumpMapShader(const D3DBumpMapShader& other)
{
}
...
bool D3DBumpMapShader::Init(ID3D11Device* device, HWND hwnd)
{
	// Initialize the vertex and pixel shaders.
	if(!InitShader(device, hwnd, L"Data/Shaders/bumpmap.vs", L"Data/Shaders/bumpmap.ps"))
		return false;
...
bool D3DBumpMapShader::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
void D3DBumpMapShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	CreateDirectory(L"Error Logs", NULL); // Create the directory for error messages, if it already exists, just ignore the return.
	char* compileErrors;
	char* filename = "Error Logs/BumpMapShader-error-";
...
void D3DBumpMapShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
