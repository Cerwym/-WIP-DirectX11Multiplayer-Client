#include "D3DLightShader.h"


D3DLightShader::D3DLightShader()
{
	m_vertexShader = 0;
	m_pixelShader = 0;
	m_layout = 0;
...
D3DLightShader::~D3DLightShader()
{
	...
	S_RELEASE(m_lightBuffer)
	S_RELEASE(m_cameraBuffer);
...
bool D3DLightShader::Init(ID3D11Device* device, HWND hwnd)
{
	bool result;


	// Init the vertex and pixel shaders.
	result = InitShader(device, hwnd, L"Data/Shaders/light.vs", L"Data/Shaders/light.ps");
	if(!result)
		return false;
...
bool D3DLightShader::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	// The addition of the normal element is a significant change from the other shader classes
	...
	cameraBufferDesc.ByteWidth = sizeof(CameraBufferType); //Todo: investigate round this up to 16 bytes to reduce redundant data
...
void D3DLightShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	CreateDirectory(L"Error Logs", NULL); // Create the directory for error messages, if it already exists, just ignore the return.
	char* compileErrors;
	char* filename = "Error Logs/LightShader-error-";
	...
	XMFLOAT4 ambientColour, XMFLOAT4 diffuseColor, XMFLOAT3 cameraPosition, XMFLOAT4 specularColour, float specularPower)
{
	...
	// Lock the camera's constant buffer so that it can be written to.
	...
	bufferNumber = 1; // The value is now changed to 1 as the camera is the SECOND buffer in the vertex shader

	// Now set the camera constant buffer int he vertex shader with the updated values
...
void D3DLightShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
